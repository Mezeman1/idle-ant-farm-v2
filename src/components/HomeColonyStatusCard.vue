...
<template>
  <section
    :class="['bg-gradient-to-br from-amber-100 to-amber-50 dark:from-amber-900/40 dark:to-amber-800/30 rounded-xl p-3 shadow-md', className]">
    <h2 class="text-base font-bold mb-2 flex items-center">
      <span class="i-heroicons-chart-bar text-amber-700 dark:text-amber-500 mr-2"></span>
      Colony Status
    </h2>

    <div class="grid grid-cols-3 gap-2">
      <div
        class="bg-white/80 dark:bg-gray-800/80 p-2 rounded-lg shadow-sm border border-amber-200 dark:border-amber-700">
        <div class="text-xs text-amber-700 dark:text-amber-400 font-medium">Food</div>
        <div class="text-sm font-bold flex items-center">
          <span class="i-heroicons-cake text-amber-600 dark:text-amber-500 mr-1 text-xs"></span>
      ...
      <div
        class="bg-white/80 dark:bg-gray-800/80 p-2 rounded-lg shadow-sm border border-amber-200 dark:border-amber-700">
        <div class="text-xs text-amber-700 dark:text-amber-400 font-medium">Food/Trip</div>
        <div class="text-sm font-bold flex items-center">
          <span class="i-heroicons-arrow-trending-up text-amber-600 dark:text-amber-500 mr-1 text-xs"></span>
      ...
      <div
        class="bg-white/80 dark:bg-gray-800/80 p-2 rounded-lg shadow-sm border border-amber-200 dark:border-amber-700">
        <div class="text-xs text-amber-700 dark:text-amber-400 font-medium">Worker Ants</div>
        <div class="text-sm font-bold flex items-center">
          <span class="i-heroicons-bug-ant text-amber-600 dark:text-amber-500 mr-1 text-xs"></span>
          {{ generatorStore.formatGeneratorCount('worker') }}
        </div>
        <div class="text-xs text-amber-600 dark:text-amber-500" v-if="hasAutoGeneratedWorkers">
    ...
    <div class="grid grid-cols-2 gap-2 mt-2">
      <div
        class="bg-white/80 dark:bg-gray-800/80 p-2 rounded-lg shadow-sm border border-amber-200 dark:border-amber-700">
        <div class="text-xs text-amber-700 dark:text-amber-400 font-medium flex justify-between">
        ...
        <div class="text-xs text-amber-600 dark:text-amber-500 mt-1">
          {{ formatDecimal(prestigeStore.currentLoopTicks, 0) }} / {{
      ...
      <div
        ...
        <div class="text-xs text-amber-700 dark:text-amber-400 font-medium flex justify-between">
          ...
          <span>{{ Math.min(100,
            Math.round((generatorStore.food.div(prestigeStore.foodForNextLoop).toNumber()) * 100))
          }}%</span>
        </div>
        <div class="h-1.5 bg-amber-100 dark:bg-amber-900/50 rounded-full mt-1 overflow-hidden">
          <div class="h-full bg-amber-500 dark:bg-amber-400 transition-all duration-300 ease-out"
    ...
    <div class="flex gap-2 mt-2">
      <div
        class="bg-amber-50 dark:bg-amber-900/30 p-2 rounded-lg border border-amber-200 dark:border-amber-700 flex-grow text-xs text-amber-800 dark:text-amber-300">
        <span class="i-heroicons-information-circle text-amber-600 dark:text-amber-500 mr-1 inline-block"></span>
